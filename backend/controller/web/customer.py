...
def getCustomer(cursor,customerid):
    try:
        result=getOneQuery(cursor,'SELECT id,first_name,last_name,email from customers WHERE id=?',(customerid,))
        if not result:
...
def updateCustomer(mysql,cursor,data,customer_id):
    try:
        if "email" in data:
            result=getOneQuery(cursor,'SELECT email from customers WHERE email=? AND id != ?',(data['email'],customer_id))
            if   result:
        ...
        for k, v in data.items():
            ...
            fields += k + '="'+v+'",'
        result=execQuery(mysql,cursor,'UPDATE  customers  SET '+fields.rstrip(',')+' WHERE id=?',(customer_id,))
...
def getCustomersWithPagination(limit,page,cursor):
    try:
        ...
        data=getAllQueryWithCondition(cursor,'SELECT id,email,first_name,last_name,is_active from customers  LIMIT ? OFFSET ?',(int(limit),int(offset)))
        if not data:
            message=getMessage('CUSTOMER_NOT_FOUND')
            return message
...
def activeDeativeCustomer(mysql,cursor,customer_id,is_active):
    try:
        result=getOneQuery(cursor,'SELECT email from customers WHERE id = ?',(customer_id))
        if  result:   
            result=execQuery(mysql,cursor,'UPDATE  customers  SET is_active=? WHERE id=?',(is_active,customer_id))
            flash(getMessage('CUSTOMER_UPDATED_SUCCESSFULLY'))
            return {"updated":True }
...
def deleteCustomer(mysql,cursor,data):
    try:
        exist=getOneQuery(cursor,'SELECT email from customers WHERE id=?',(data['customer_id'],))
        if exist:
             execQuery(mysql,cursor,'DELETE from customers WHERE id=?',(data['customer_id'],))
